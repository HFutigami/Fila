...
if 'connection' not in st.session_state:
    ...
    def create_df_prioridades(df):
        ...
        df_prioridades = df[(df['CAIXA'].isin(prioridade['CAIXAS'])) | (df['NUM OS'].isin(prioridade['CAIXAS']))].copy()
        
        df_prioridades['FILA'] = 0
        df_prioridades['SAÍDA'] = 0
        df_prioridades.loc[~df['ENDEREÇO'].isin(['LAB', 'EQUIPE TECNICA', 'GESTAO DE ATIVOS', 'QUALIDADE', 'RETRIAGEM']), 'FILA'] = 1
        df_prioridades.loc[df['ENDEREÇO'].isin(['LAB', 'EQUIPE TECNICA', 'GESTAO DE ATIVOS', 'QUALIDADE', 'RETRIAGEM']), 'SAÍDA'] = 1
