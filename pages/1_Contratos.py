...
if 'connection' not in st.session_state:
    ...
    def create_df_terceiros_contratos(df):
        df_saldo_atual_contratos = df.copy()
        print(df_saldo_atual_contratos)
        df_saldo_atual_contratos = df_saldo_atual_contratos[
    ...
    def open_dialog_filtros_saldo():
        ...
        if st.button('APLICAR FILTROS', use_container_width=True):
            ...
            df = df[(df['ENTRADA GERFLOOR'] >= pd.to_datetime(ft_dtger_min)) & (
                        df['ENTRADA GERFLOOR'] <= pd.to_datetime(ft_dtger_max))]
            df = df[(df['ENTRADA FILA'] >= pd.to_datetime(ft_dtfila_min)) & (
                        df['ENTRADA FILA'] <= pd.to_datetime(ft_dtfila_max))]
    ...
    def open_dialog_filtros_saida():
        ...
        if st.button('APLICAR FILTROS', use_container_width=True):
            ...
            df = df[(df['ENTRADA FILA'] >= pd.to_datetime(ft_dtfila_min)) & (
                        df['ENTRADA FILA'] <= pd.to_datetime(ft_dtfila_max))]
            df = df[(df['SAÍDA FILA'] >= pd.to_datetime(ft_dtsfila_min)) & (
                        df['SAÍDA FILA'] <= pd.to_datetime(ft_dtsfila_max))]
    ...
    tabs_saldo, tabs_saida, tabs_terceiros, tabs_geral = st.tabs(['Saldo', 'Saídas', 'Terceiros', 'Tabela Geral'])
    ...
    if 'df_saldo_atual_contratos' not in st.session_state or 'df_saldo_atual_contratos_resumido' not in st.session_state:
        ...
        st.session_state['df_saldo_atual_contratos_resumido'] = create_df_saldo_contratos_resumido(
            st.session_state['df_saldo_atual_contratos'])
    ...
    if saldo_atual_contratos.selection.rows:
        df_saldo_atual_contratos_resumido['CONCATENADO'] = df_saldo_atual_contratos_resumido['CLIENTE'] + \
                                                           df_saldo_atual_contratos_resumido['EQUIPAMENTO']
        df_saldo_atual_contratos['CONCATENADO'] = df_saldo_atual_contratos['CLIENTE'] + df_saldo_atual_contratos[
            'EQUIPAMENTO']
        filtro_saldo = list(df_saldo_atual_contratos_resumido.iloc[saldo_atual_contratos.selection.rows]['CONCATENADO'])
        saldo_atual_contratos_selecao = df_saldo_atual_contratos[
            df_saldo_atual_contratos['CONCATENADO'].isin(filtro_saldo)]
        st.session_state['saldo_atual_contratos_selecao'] = saldo_atual_contratos_selecao
        r0c1.metric('Total de equipamentos (seleção)',
                    '{:,}'.format(
                        sum(df_saldo_atual_contratos_resumido.iloc[saldo_atual_contratos.selection.rows]['QTD FILA']) +
                        sum(df_saldo_atual_contratos_resumido.iloc[saldo_atual_contratos.selection.rows][
                                'QTD OS'])).replace(',', '.'))
        r0c2.metric('Equipamentos aguardando OS (seleção)',
                    '{:,}'.format(sum(df_saldo_atual_contratos_resumido.iloc[saldo_atual_contratos.selection.rows][
                                          'QTD OS'])).replace(',', '.'))
    ...
    if 'df_saidas_contratos' not in st.session_state or 'df_saidas_contratos_resumido' not in st.session_state:
        ...
        st.session_state['df_saidas_contratos_resumido'] = create_df_saidas_contratos_resumido(
            st.session_state['df_saidas_contratos'])
    ...
    if saidas_contratos.selection.rows:
        df_saidas_contratos_resumido['CONCATENADO'] = df_saidas_contratos_resumido['CLIENTE'] + \
                                                      df_saidas_contratos_resumido['EQUIPAMENTO']
        ...
        t2r0c1.metric('Total de saídas (seleção)',
                      '{:,}'.format(len(saidas_contratos_selecao['SERIAL'])).replace(',', '.'))
        if len(saidas_contratos_selecao[saidas_contratos_selecao['SAÍDA FILA'] >= datetime.today() - timedelta(
                hours=datetime.today().hour + 1)]) > 0:
            filtro_ontem = ((saidas_contratos_selecao['SAÍDA FILA'] >= datetime.today() - timedelta(days=1,
                                                                                                    hours=datetime.today().hour,
                                                                                                    minutes=datetime.today().minute)) &
                            (saidas_contratos_selecao['SAÍDA FILA'] <= datetime.today() - timedelta(
                                hours=datetime.today().hour + 1, minutes=datetime.today().minute)))
            try:
                t2r0c2.metric('Saídas do dia (seleção)', '{:,}'.format(len(saidas_contratos_selecao[
                                                                               saidas_contratos_selecao[
                                                                                   'SAÍDA FILA'] >= datetime.today() - timedelta(
                                                                                   hours=datetime.today().hour + 1)])).replace(
                    ',', '.'),
                              delta='{:.2%}'.format(((len(saidas_contratos_selecao[saidas_contratos_selecao[
                                                                                       'SAÍDA FILA'] >= datetime.today() - timedelta(
                                  hours=datetime.today().hour + 1)])) - len(
                                  saidas_contratos_selecao[filtro_ontem])) / len(
                                  saidas_contratos_selecao[filtro_ontem])))
            except:
                t2r0c2.metric('Saídas do dia (seleção)', '{:,}'.format(len(saidas_contratos_selecao[
                                                                               saidas_contratos_selecao[
                                                                                   'SAÍDA FILA'] >= datetime.today() - timedelta(
            ...
            t2r0c2.metric('Saídas do dia (seleção)', 0)
    else:
        t2r0c1.metric('Total de saídas',
                      '{:,}'.format(sum(df_saidas_contratos_resumido['QUANTIDADE'])).replace(',', '.'))
        if len(df_saidas_contratos[df_saidas_contratos['SAÍDA FILA'] >= datetime.today() - timedelta(
                hours=datetime.today().hour + 1)]) > 0:
            filtro_ontem = ((df_saidas_contratos['SAÍDA FILA'] >= datetime.today() - timedelta(days=1,
                                                                                               hours=datetime.today().hour,
                                                                                               minutes=datetime.today().minute)) &
                            (df_saidas_contratos['SAÍDA FILA'] <= datetime.today() - timedelta(
                                hours=datetime.today().hour, minutes=datetime.today().minute)))
            try:
                t2r0c2.metric('Saídas do dia', '{:,}'.format(len(df_saidas_contratos[df_saidas_contratos[
                                                                                         'SAÍDA FILA'] >= datetime.today() - timedelta(
                    hours=datetime.today().hour + 1)])).replace(',', '.'),
                              delta='{:.2%}'.format(((len(df_saidas_contratos[df_saidas_contratos[
                                                                                  'SAÍDA FILA'] >= datetime.today() - timedelta(
                                  hours=datetime.today().hour + 1)])) - len(df_saidas_contratos[filtro_ontem])) / len(
                                  df_saidas_contratos[filtro_ontem])))
            except:
                t2r0c2.metric('Saídas do dia', '{:,}'.format(len(df_saidas_contratos[df_saidas_contratos[
                                                                                         'SAÍDA FILA'] >= datetime.today() - timedelta(
                    hours=datetime.today().hour + 1)])).replace(',', '.'),
    ...
    if 'df_terceiros_contratos' not in st.session_state or 'df_terceiros_contratos_resumido' not in st.session_state:
        ...
        st.session_state['df_terceiros_contratos_resumido'] = create_df_terceiros_contratos_resumido(
            st.session_state['df_terceiros_contratos'])
    ...
    if terceiros_contratos.selection.rows:
        filtro_saldo = list(df_terceiros_contratos_resumido.iloc[terceiros_contratos.selection.rows]['TERCEIROS'])
    ...
    if 'terceiros_contratos_selecao' in st.session_state and terceiros_contratos.selection.rows:
        t3r1c2.write('Classificação dos equipamentos em posse de terceiros de acordo com % do SLA.')
